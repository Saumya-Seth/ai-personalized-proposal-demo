...
export default function SpectrumReachProposal() {
  ...
  const [focusSplit, setFocusSplit] = useState({ women3564: 50, adults55: 30, general: 20 });
  const [selectedTab, setSelectedTab] = useState('overview');

  const channelReach = useMemo(() => {
    const factor = budget / 50000; // normalize to $50k baseline
    return baseChannels.map(c => ({ ...c, reach: Math.round(c.reach * factor * (flightWeeks / 4)) }));
  }, [budget, flightWeeks]);

  const investmentMix = useMemo(() => {
    return baseInvestment.map(i => ({ ...i, dollars: Math.round((i.pct / 100) * budget) }));
  }, [budget]);

  const estimatedImpact = useMemo(() => {
    const totalImpr = channelReach.reduce((s, c) => s + c.reach, 0);
    const impressionsToVisit = 1200; // conservative model: 1 visit per 1.2k impressions
    const impressionsToPatient = 25000; // 1 new patient per 25k impressions (illustrative)
  ...
  const pieData = [
    { name: 'Women 35–64', value: focusSplit.women3564 },
  ...
  // Keep audience split totaling 100
  function updateSplit(part, newVal) {
    ...
    const updated = { ...focusSplit, [part]: newVal };
    otherKeys.forEach(k => { updated[k] = Math.round((focusSplit[k] / otherTotal) * remaining); });
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900" aria-label="Spectrum Reach ROC Proposal Microsite">
      <main className="max-w-6xl mx-auto p-6">
        ...
        {/* HERO */}
        <header className="rounded-2xl overflow-hidden shadow-xl" style={{ background: `linear-gradient(120deg, ${BRAND.primary}, ${BRAND.secondary} 55%, ${BRAND.accent})` }}>
          <div className="p-8 md:p-12 text-white">
            <div className="flex items-start justify-between gap-6">
              <div className="max-w-2xl">
                <div className="uppercase text-xs tracking-widest text-sky-100 font-bold">Spectrum Reach × Reno Orthopedic Centre</div>
                <h1 className="text-3xl md:text-5xl mt-2 font-extrabold leading-tight">Strategic Multiscreen Proposal for ROC</h1>
                <p className="mt-3 text-sm md:text-base opacity-90">
                  A data-driven plan spanning Linear TV, Streaming/CTV, Display, and Retargeting. Powered by Spectrum Reach first‑party data and measured through AudienceTrak.
                </p>
              ...
              <div className="text-right shrink-0">
                ...
                <div className="mt-4 flex gap-3 justify-end">
                  <button onClick={() => setSelectedTab('overview')} className={`px-4 py-2 rounded-lg text-sm font-semibold ${selectedTab==='overview' ? 'bg-white text-blue-800' : 'bg-white/10 text-white'}`}>Overview</button>
                  <button onClick={() => setSelectedTab('plan')} className={`px-4 py-2 rounded-lg text-sm font-semibold ${selectedTab==='plan' ? 'bg-white text-blue-800' : 'bg-white/10 text-white'}`}>Plan</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <aside className="col-span-1 bg-white p-5 rounded-xl shadow-md border border-slate-100">
            ...
            <div className="mt-4">
              ...
              <input type="range" min="20000" max="250000" step="5000" value={budget} onChange={(e)=>setBudget(Number(e.target.value))} className="w-full mt-3" aria-label="Campaign budget" />
              <div className="flex justify-between text-xs text-slate-500 mt-1"><span>$20k</span><span>$250k</span></div>
            </div>

            <div className="mt-4">
              <label className="text-sm font-medium">Flight Length — {flightWeeks} weeks</label>
              <input type="range" min="2" max="16" step="1" value={flightWeeks} onChange={(e)=>setFlightWeeks(Number(e.target.value))} className="w-full mt-3" aria-label="Flight length" />
              <div className="flex justify-between text-xs text-slate-500 mt-1"><span>2w</span><span>16w</span></div>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 space-y-2">
                <div>
                  <div className="flex justify-between text-xs"><span>Women 35–64</span><strong>{focusSplit.women3564}%</strong></div>
                  <input type="range" min="0" max="100" value={focusSplit.women3564} onChange={(e)=>updateSplit('women3564', Number(e.target.value))} className="w-full mt-2" aria-label="Women 35–64 focus" />
                </div>
                <div>
                  <div className="flex justify-between text-xs"><span>Adults 55+</span><strong>{focusSplit.adults55}%</strong></div>
                  <input type="range" min="0" max="100" value={focusSplit.adults55} onChange={(e)=>updateSplit('adults55', Number(e.target.value))} className="w-full mt-2" aria-label="Adults 55+ focus" />
                </div>
              </div>
            ...
            <div className="mt-5 flex gap-2">
              <button onClick={downloadPDF}  className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg font-semibold bg-gradient-to-r from-[#001b33] to-[#137EBD] text-white shadow" aria-label="Print or save PDF"> <Printer size={16}/> Print / Save PDF</button>
              <button onClick={()=>alert('Download package (assets + summary) — connect to backend to enable.')} className="px-3 py-2 rounded-lg border border-slate-200" aria-label="Export package"> <Download size={14}/> Export</button>
            </div>

            <div className="mt-6 text-xs text-slate-600">
              <div><strong>Total Impressions:</strong> {estimatedImpact.totalImpr.toLocaleString()}</div>
              <div><strong>Estimated Site Visits:</strong> {estimatedImpact.visits.toLocaleString()}</div>
              <div><strong>Est. New Patients:</strong> {estimatedImpact.patients.toLocaleString()}</div>
              <div><strong>Cost per Visit:</strong> ${estimatedImpact.cpv.toLocaleString()}</div>
            </div>
          </aside>
          ...
          <section className="col-span-2 bg-white p-6 rounded-xl shadow-md border border-slate-100">
            <nav className="flex gap-3 mb-4">
              <button onClick={()=>setSelectedTab('overview')} className={`px-3 py-1 rounded-full text-sm font-semibold ${selectedTab==='overview' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`}>Overview</button>
              <button onClick={()=>setSelectedTab('plan')} className={`px-3 py-1 rounded-full text-sm font-semibold ${selectedTab==='plan' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`}>Plan</button>
              <button onClick={()=>setSelectedTab('measurement')} className={`px-3 py-1 rounded-full text-sm font-semibold ${selectedTab==='measurement' ? 'bg-slate-100 text-slate-900' : 'text-slate-500'}`}>Measurement</button>
            </nav>

            {selectedTab==='overview' && (
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Campaign Overview</h2>
                <p className="mt-2 text-slate-600">This plan unifies Spectrum Reach’s Linear + CTV scale with precision digital to reach Women 35–64 and Adults 55+, driving clinic visits and patient inquiries for ROC.</p>
                    ...
                    <div style={{ width: '100%', height: 280 }}>
                      <ResponsiveContainer width="100%" height={280}>
                        <PieChart aria-label="Audience focus pie chart">
                          <Pie data={pieData} dataKey="value" nameKey="name" innerRadius={48} outerRadius={90} label={({percent}) => `${Math.round(percent*100)}%`}>
            ...
            {selectedTab==='plan' && (
              <div>
                ...
                <div className="mt-6 p-4 rounded-lg border">
                  ...
                  <ol className="mt-3 ml-5 text-slate-600 list-decimal">
                    <li><b>Smart Audience Shifts</b> — Move investment dynamically toward high-performing segments.</li>
                    <li><b>Geo:</b> Reno DMA with city overlays; clinic radii for retargeting pools.</li>
                    <li><b>Optimization: </b>Cross‑screen frequency caps; AudienceTrak pacing & lift.</li>
                  </ol>
                </div>
                ...
                <div className="mt-6 p-4 rounded-lg border">
                  ...
                  <table className="w-full text-sm mt-2 border-collapse">
                    <thead>
                      <tr className="bg-slate-50"><th className="p-2 text-left">Channel</th><th className="p-2 text-left">Focus</th><th className="p-2 text-left">Estimate</th></tr>
                    </thead>
                    <tbody>
                      <tr><td className="p-2">Linear TV</td><td className="p-2">Awareness</td><td className="p-2">~100k–150k HH/week</td></tr>
                      <tr className="bg-slate-50"><td className="p-2">Streaming / CTV</td><td className="p-2">Engagement</td><td className="p-2">~220k impressions (baseline)</td></tr>
                      <tr><td className="p-2">Digital Display</td><td className="p-2">Consideration</td><td className="p-2">~150k impressions</td></tr>
                      <tr className="bg-slate-50"><td className="p-2">Retargeting</td><td className="p-2">Conversion</td><td className="p-2">~60k impressions</td></tr>
                      <tr><td className="p-2">Paid Social</td><td className="p-2">Support</td><td className="p-2">~90k reach</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-slate-600 mt-2">Match back site visits and conversions by exposure cohort; leverage geo‑based lift for Linear plus CTV view‑through.</p>
                  </div>
                </div>
        ...
        <section className="mt-12 bg-white p-8 rounded-2xl shadow-lg border border-slate-100">
          <div className="flex items-start gap-8 flex-col md:flex-row">
            <div className="flex-1">
              <h3 className="text-3xl font-extrabold bg-gradient-to-r from-[#001b33] via-[#137EBD] to-[#0099d8] bg-clip-text text-transparent leading-tight">Ready to Launch with Spectrum Reach?</h3>
            ...
            <aside className="w-full md:w-72 p-4 border rounded-lg">
              ...
              <div className="mt-3">
                <button onClick={()=>alert('Add your scheduling link')}  className="w-full mt-3 py-2 rounded-lg border text-white" style={{ backgroundColor: BRAND.primary, borderColor: BRAND.primary }}>Schedule Kickoff</button>
                <button onClick={downloadPDF} className="w-full mt-3 py-2 rounded-lg border text-[#001b33]" style={{ backgroundColor: BRAND.accent, borderColor: BRAND.accent }}>Save as PDF</button>
              </div>
            </aside>
          </div>
        </section>
